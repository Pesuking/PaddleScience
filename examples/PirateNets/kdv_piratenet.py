...
def train(cfg: DictConfig):
    model = ppsci.arch.PirateNet(**cfg.MODEL)  # set model

    equation = {"KDV": ppsci.equation.KDV(eta=1, mu=0.022)}

    data = sio.loadmat(cfg.DATA_PATH)  # load data
    ...
    pde_constraint = ppsci.constraint.SupervisedConstraint(
        ...
        output_expr=equation["KDV"].equations,
    ...
    #         "x": paddle.to_tensor(T, dtype=dtype),
    #     },
    #     {"u": paddle.to_tensor(Y, dtype=dtype)},
    # )
...
@hydra.main(version_base=None, config_path="./conf", config_name="kdv_piratenet.yaml")
